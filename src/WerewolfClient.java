		...
		public void run() {
			...
			while (true) {
				...
				* サーバーがクライアントにwrite()したらデータが送られてきて、buffにデータが入って、次の行に実行が進む*/
				...
				//サーバーから“.nameset.@@@”というデータが送られてきたら、nameに@@@を代入する
				if(readStr.regionMatches(0,".nameset.",0,8)) {
					name = new String(Arrays.copyOfRange(buff, 9, n));
				}
				//サーバーから“.roleset.@@@”と送られたら@@@を代入。
				if(readStr.regionMatches(0,".roleset.",0,8)){
					role = new String(Arrays.copyOfRange(buff, 9, n));
				}
				if(readStr.regionMatches(0,".text.",0,5)) {
					//コンソールに文字列を表示
					System.out.write(buff, 6, n-6);
				}
			}
		}
	}
